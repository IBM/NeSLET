...
parser.add_argument('--file_training_params', type=str)

args = parser.parse_args()

###############################################################

tr_params=args.file_training_params
print("reading training_param_file")
try:
    with open(tr_params,'r') as fp:
        training_params = json.load(fp) 
        print("printing training_params",training_params["bert_model"])
except json.decoder.JSONDecodeError:
    with open(args.file_training_params) as fp:
        for line in fp:
            line = line.replace("'", "\"")
...
biencoder_params["bert_model"] = training_params["bert_model"]
biencoder_params["lowercase"] = training_params["lowercase"]
biencoder_params["out_dim"] = training_params["out_dim"]
biencoder_params["pull_from_layer"] = training_params["pull_from_layer"]
biencoder_params["add_linear"] = training_params["add_linear"]
biencoder_params["no_cuda"] = False
biencoder_params['silent'] = training_params['silent']
#########################################################################
...
if args.chunk_end ==-1:
    candidate_encoding = encode_candidate(
        biencoder,
        candidate_pool[args.chunk_start:],

...
####################
#This is modified version of main_dense to make the api service  work only in CPU 
...
def _run_biencoder(biencoder, dataloader, candidate_encoding, top_k=100, indexer=None, device="cuda"):
...
#def _run_crossencoder(crossencoder, dataloader, logger, context_len, device="cuda:0"):
...
#def _run_crossencoder(crossencoder, dataloader, logger, context_len):
    ...
    #res = evaluate(crossencoder, dataloader, logger, context_len, silent=False)
...
def load_models(args, logger=None, no_cuda=False):
    ...
    biencoder_params["no_cuda"] = no_cuda
    ####################################################################
    if args.biencoder_training_params is not None:
        ...
        biencoder_params["pull_from_layer"] = biencoder_training_params["pull_from_layer"]
        ...
        biencoder_params['max_context_length'] = biencoder_training_params['max_context_length']
        biencoder_params['max_cand_length'] = biencoder_training_params['max_cand_length']
    ####################################################################
    ############## DG added the following lines ##############
    biencoder_params["data_parallel"] = not no_cuda
    biencoder_params["eval_batch_size"] =1 # 128 #256 and 512 will throw CUDA out of memory error.
    ##########################################################
    ...
    if not args.fast:
        ...
        with open(args.crossencoder_config) as json_file:
            ...
            ############## DG added the following lines ##############
            crossencoder_params["data_parallel"] = not no_cuda
            crossencoder_params["eval_batch_size"] = 1 #10
            ##########################################################
...
def run(
    ...
    while not stopping_condition:
        ...
        labels, nns, scores = _run_biencoder(
            biencoder, dataloader, candidate_encoding, top_k, faiss_indexer, device=device
        )
